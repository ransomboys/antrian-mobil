...
void keluar(int plt, struct queue *q,struct stack *s)
{
	...
	i=q->front;
	if(q->count==0){
		printf("\nAntrian kosong, Gaskeun slur.. :v !\n");
		getch();
		return;
	}
	else if(cek_mobil(plt,q)==0){
		printf("\nPlat mobil yang lu masukin gk ada di dalem antrian !\n");
		getch();
		return;
	}
...
void tampil(struct queue *q)
{
	...
	printf("\n\n**Tekan sembarang tombol untuk kembali ke menu awal**");
	getch();
	return;
}
...
void main()
{
	...
	printf("===========================\n");
	printf("===========================\n");
	printf("   TOOLS ANTRIAN MOBIL\n");
	printf("===========================\n");
	printf("Author: Pennywise\n");
	printf("Thank to: Allah S.W.T\n");
	printf("===========================\n");
	printf("1. Masukkan mobil\n2. Keluarkan mobil\n3. Tampilkan antrian\n");
	printf("4. Keluar\n");
	printf("===========================\n");
	printf("@root:: Masukan pilihan anda: -->"); scanf("%d",&jawab);
	printf("===========================\n");

	if(jawab==1){
		printf("Masukkan plat nomor mobil yg masuk (nomornya aja, jangan pake huruf) : "); scanf("%d",&plt);
		masuk(plt,&q);
		tampil(&q);
	}
	else if(jawab==2){
		printf("Masukkan plat nomor mobil yg keluar (nomornya aja, jangan pake huruf):"); scanf("%d",&plt);
		keluar(plt,&q,&s);
		tampil(&q);
	}
	else if(jawab==3){
		...
		printf("\n\nLu milih apasih cok!\n");
		printf("\n\nGk ada dalam menunya!\n");
		printf("\n\nPilih lagi yang teliti :P\n");
		getch();
	}
	}while(jawab!=4);
}
